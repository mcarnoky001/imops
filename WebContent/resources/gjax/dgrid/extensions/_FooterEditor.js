...
define([
	...
	"dijit/form/Button",
	"dijit/form/Form"
], function(declare, lang, array, domStyle, query, on, at, sync, converters, put, i18n, ValidationTextBox, Button, Form) {
	return declare(null, {
		...
		_createEditorColumns : function() {
			this._editorColumns = array.map(this._subRows || this.subRows, function(subRow) {
				return array.map(subRow, function(column) {//set id base on field for each column
					...
					var rels = footerColumn.field.split(".");
					...
					if (footerColumn.field === this.__actionButtonColumnId) {//use already existing actionButtonColumn (override edit, delete with save, reset buttons) 
						...
						if (editor !== false) {
							...
							if (footerColumn.editor.prototype.declaredClass === "dijit.form.CheckBox") {/* git-qa */ //different binding for CheckBoxes
								footerColumn.editorArgs.checked = at("rel:", footerColumn.field);
							}
							if (footerColumn.editor.prototype.declaredClass === "dijit.form.CurrencyTextBox") {//apply transform fnc for CurrencyTextBox
								footerColumn.editorArgs.value = at("rel:" + objRel, propRel).transform(converters.currency);
							}
						}
					}
					return footerColumn;
