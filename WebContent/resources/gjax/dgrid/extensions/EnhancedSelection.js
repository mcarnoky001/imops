define([
	...
	return declare(Selection, {
		...
		getSelectedItems : function() {
			...
			for ( var id in this.selection) {
				if (this.selection[id]) {
					selected.push(this.row(id).data || this._selectedItems[id]);
				}
			}
			return selected;
		},
		...
		_createSelectedRecordNode : function() {
			...
			var prevNode = this.paginationStatusNode || this._totalNode;
		...
		startup : function() {
			...
			if (this.selectionMode == "radio" || (this.preselectFirstRow && this.selectionMode != "none")) {
				this.on(eventUtil.refreshCompleteData, lang.hitch(this, function(items) {
					if (!this.deselectOnRefresh && df.keys(this.selection).length) {
						return;
					}
