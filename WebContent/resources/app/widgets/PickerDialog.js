define([
	...
	"dojox/mobile/Button",
	"xstyle/css!./PickerDialog.css"
], function(declare, SimpleDialog, _TemplatedMixin, _WidgetsInTemplateMixin, template,//
i18n, Memory, lang, debounce, error, when, whitelistMixin) {
	...
	return declare([
		...
		startup : function() {
			...
			this.list.on("checkStateChanged", lang.hitch(this, function(listItem, newState) {
				if (newState) {
					this.emit("item-select", {
						item : whitelistMixin(this.itemProps, {}, listItem)
					});
					this.hide();
				}
			}));
